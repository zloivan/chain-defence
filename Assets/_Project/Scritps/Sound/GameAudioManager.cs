...
namespace ChainDefense.Sound
{
    public class GameAudioManager : SingletonBehaviour<GameAudioManager>
    {
        ...
        [FormerlySerializedAs("_towerShootPoisonSound")] [SerializeField]
        private SoundData _towerShootExplodeSound;
        ...
        {
            base.OnDestroy();
            EventBus<BallDragStartedEvent>.Deregister(_ballDragStartedBinding);
            EventBus<ChainDestroyedEvent>.Deregister(_chainDestroyedBinding);
            EventBus<ButtonClickedEvent>.Deregister(_buttonClickedBinding);
            EventBus<TowerAttackEvent>.Deregister(_towerAttackBinding);
            EventBus<EnemyDestroyedEvent>.Deregister(_enemyDestroyedBinding);
            EventBus<EnemyTakeDamageEvent>.Deregister(_enemyTakeDamageBinding);
            EventBus<BaseTakeDamageEvent>.Deregister(_baseTakeDamageBinding);
            EventBus<BaseDestroyedEvent>.Deregister(_gameOverBinding);
            EventBus<WaveSpawnedEvent>.Deregister(_waveSpawnedBinding);
            EventBus<LevelCompletedEvent>.Deregister(_levelCompletedBinding);
            EventBus<InvalidChainEvent>.Deregister(_invalidChainBinding);
        }

        private void OnBallDragStarted(BallDragStartedEvent evt) =>
            PlaySound(_ballSelectSound);

        private void OnChainDestroyed(ChainDestroyedEvent evt) =>
            PlaySound(_chainCompleteSound);
        ...
        private void OnEnemyDestroyed(EnemyDestroyedEvent evt) =>
            PlaySound(_enemyDeathSound);

        private void OnEnemyTakeDamage(EnemyTakeDamageEvent evt) =>
            PlaySound(_enemyHitSound);

        private void OnBaseTakeDamage(BaseTakeDamageEvent evt) =>
            PlaySound(_baseDamageSound);

        private void OnGameOver(BaseDestroyedEvent evt) =>
            PlaySound(_defeatSound);

        private void OnWaveSpawned(WaveSpawnedEvent evt) =>
            PlaySound(_waveStartSound);

        private void OnLevelCompleted(LevelCompletedEvent evt) =>
            PlaySound(_victorySound);

        private void OnInvalidChain(InvalidChainEvent evt) =>
            PlaySound(_invalidChainSound);
